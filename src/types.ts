...
export type BumpyOptions<
    T extends Collection<string, z.ZodSchema>> = {
        basePath: string,
...
export type Data<T extends Collection<string, z.ZodSchema>> = Prettify<Record<T["name"], Prettify<Record<string, WithBaseField<InferSchema<T['schema']>>>>>>

export type CollectionOperations<T extends Collection<string, z.ZodSchema>> = {
    [K in T['name']]: T extends Collection<K, infer S extends z.ZodSchema> ? Operations<S> : never
}
